...
def connect():
    ...
    p.resetDebugVisualizerCamera(cameraDistance=1, cameraYaw=90, cameraPitch=-40, cameraTargetPosition=[.7, 0, -0.3], physicsClientId=clid)
...
def connect_headless(gui=False):
    ...
    p.resetDebugVisualizerCamera(cameraDistance=1, cameraYaw=90, cameraPitch=-40, cameraTargetPosition=[.7, 0, -0.3], physicsClientId=cid)
    #p.resetDebugVisualizerCamera(0.8, 90, -20, [0.75, -.2, 0])
...
def setup_headless(timestep=1./240, solver_iterations=150, gravity=-10, physicsClientId=0):
    ...
    '''
    p.setPhysicsEngineParameter(numSolverIterations=solver_iterations, physicsClientId=physicsClientId)
...
def load_urdf_randomize_color(filepath, pos=[0, 0, 0], quat=[0, 0, 0, 1], scale=1, rgba=None, physicsClientId=0):
    ...
    if rgba is not None:
        ...
        try:
            ...
            p.changeVisualShape(body, -1, rgbaColor=rgba, physicsClientId=physicsClientId)
        finally:
            pass
            # os.remove(rand_filepath)
    else:
        body = p.loadURDF(filepath, globalScaling=scale, physicsClientId=physicsClientId)
...
def load_obj(filepathcollision, filepathvisual, pos=[0, 0, 0], quat=[0, 0, 0, 1], scale=1, rgba=None, physicsClientId=0):
    collisionid= p.createCollisionShape(p.GEOM_MESH, fileName=filepathcollision, meshScale=scale * np.array([1, 1, 1]), physicsClientId=physicsClientId)
    visualid = p.createVisualShape(p.GEOM_MESH, fileName=filepathvisual, meshScale=scale * np.array([1, 1, 1]), physicsClientId=physicsClientId)
    body = p.createMultiBody(0.05, collisionid, visualid, physicsClientId=physicsClientId)
    if rgba is not None:
        p.changeVisualShape(body, -1, rgbaColor=rgba, physicsClientId=physicsClientId)
    p.resetBasePositionAndOrientation(body, pos, quat, physicsClientId=physicsClientId)
    return body
...
def get_projection_matrix(height, width, fov=60, near_plane=0.1, far_plane=2, physicsClientId=0):
    aspect = width / height
    projection_matrix = p.computeProjectionMatrixFOV(fov, aspect, near_plane, far_plane, physicsClientId=physicsClientId)
...
def quat_to_deg_batch(quat_batch, physicsClientId=0):
    ...
    for i in range(quat_batch.shape[0]): 
        euler_rad = p.getEulerFromQuaternion(quat_batch[i], physicsClientId=physicsClientId)
        euler_deg = rad_to_deg(euler_rad)
...
def get_link_dist(body_1, body_2, link_1=-1, link_2=-1, threshold=1, physicsClientId=0):
    points = p.getClosestPoints(body_1, body_2, threshold, link_1, link_2, physicsClientId=physicsClientId)
