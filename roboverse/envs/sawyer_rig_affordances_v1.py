...
class SawyerRigAffordancesV1(SawyerBaseEnv):
    ...
    def _load_table(self):
        ...
        if self.affordance_dict['handle_drawer']:
            ...
            if self.affordance_dict['drawer']:
                ...
                self._top_drawer = bullet.objects.drawer(quat=quat, pos=drawer_frame_pos,
                    rgba=self.sample_object_color())
                #print(np.array(get_drawer_frame_pos(self._top_drawer)) + np.array([0.183, -0.225, .009]))
                # self._top_drawer = bullet.objects.drawer(quat=quat, pos=np.array([0.6, s * 1.125, -.34]),
                #     rgba=self.sample_object_color())
            
            ## HARDCODE
            self.drawer_opening_num_ts = 100
    ...
    def move_drawer(self, print_stages=True):
        self.grip = -1
        ee_pos = self.get_end_effector_pos()
        ee_yaw = self.get_end_effector_theta()[2]

        drawer_handle_pos = self.get_object_pos('drawer_handle')
        drawer_frame_pos = get_drawer_frame_pos(self._top_drawer)
        ## HARDCODE to open
        #drawer_handle_goal_pos = drawer_frame_pos + td_open_coeff * np.array([np.sin(self.drawer_yaw * np.pi / 180) , -np.cos(self.drawer_yaw * np.pi / 180), 0])
        drawer_handle_goal_pos = drawer_frame_pos + td_close_coeff * np.array([np.sin(self.drawer_yaw * np.pi / 180) , -np.cos(self.drawer_yaw * np.pi / 180), 0])
        ...
        done = np.linalg.norm(drawer_handle_goal_pos[:2] - drawer_handle_pos[:2]) < 0.025
        ...
        # Stage 4: lower gripper around handle
            ...
            #print(ee_pos[2], ee_early_stage_goal_pos[2], np.linalg.norm(ee_early_stage_goal_pos[2] - ee_pos[2]))
            xy_action = (ee_early_stage_goal_pos - ee_pos) * 6
            action = np.array([xy_action[0], xy_action[1], xy_action[2]*3, 0])
        # Stage 5: open/close drawer
        else:
            if print_stages: print('Stage 5')
            xy_action = drawer_handle_goal_pos - drawer_handle_pos
            action = 6*np.array([xy_action[0], xy_action[1], 0, 0])
        
        if done:
            action = np.array([0, 0, 1, 0])

        if self.final_timestep and print_stages: print("drawer_yaw: ", self.drawer_yaw, ", drawer_frame_pos: ", drawer_frame_pos)

        ## HARDCODE
        # self.button_pos = drawer_handle_pos
        # self.button_color = self.sample_object_color()
        # self._objects['button'] = bullet.objects.button(pos=self.button_pos, rgba=self.button_color)
