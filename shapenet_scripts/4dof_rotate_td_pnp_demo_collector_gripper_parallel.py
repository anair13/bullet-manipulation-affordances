...
def collect(id):
    ...
    recon_dataset = {
        ...
        'skill_id': 5 * np.ones((args.num_trajectories_per_demo, ), dtype=np.uint8)
    }
    ...
    for j in tqdm(range(args.num_trajectories_per_demo)):
        ...
        for i in range(args.num_timesteps // 3):
            action = env.get_demo_action(first_timestep=(i == 0), final_timestep=False)
        ...
        for i in range(args.num_timesteps):
            ...
            action = env.get_demo_action(first_timestep=False, final_timestep=(i == args.num_timesteps - 1))
            next_observation, reward, done, info = env.step(action)
